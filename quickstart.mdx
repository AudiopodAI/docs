---
title: "Quick Start"
description: "Go from zero to your first API call in under 5 minutes"
---

<Info>
**TL;DR:** Register → Verify email → Create API key → Add funds → Make API calls. 
</Info>

## Try it first — no signup

Want to hear a result before creating an account? The **sandbox** issues a
short-lived key (valid ~1 hour, small built-in credit balance) so you can make a
real text-to-speech call in two requests.

<Tabs>
  <Tab title="cURL">
    ```bash
    # 1. Mint a sandbox key (no auth required)
    SANDBOX_KEY=$(curl -s -X POST "https://api.audiopod.ai/api/v1/sandbox/key" | jq -r '.key')

    # 2. Synthesize a short snippet (max 200 characters) — returns audio bytes
    curl -s -X POST "https://api.audiopod.ai/api/v1/sandbox/tts" \
      -H "Content-Type: application/json" \
      -d "{\"key\": \"$SANDBOX_KEY\", \"text\": \"Hello from the AudioPod sandbox!\"}" \
      --output sandbox.mp3
    ```
  </Tab>
  <Tab title="Python">
    ```python
    import requests

    # 1. Mint a sandbox key (no auth required)
    key = requests.post("https://api.audiopod.ai/api/v1/sandbox/key").json()["key"]

    # 2. Synthesize a short snippet (max 200 characters)
    audio = requests.post(
        "https://api.audiopod.ai/api/v1/sandbox/tts",
        json={"key": key, "text": "Hello from the AudioPod sandbox!"},
    )
    with open("sandbox.mp3", "wb") as f:
        f.write(audio.content)
    ```
  </Tab>
</Tabs>

Rather not copy-paste? The same two requests are the first folder of our
**Getting Started** Postman collection. Fork it and run them — it stores the
sandbox key for you between the two calls.

[![Run in Postman](https://run.pstmn.io/button.svg)](https://god.gw.postman.com/run-collection/58401244-916ddbda-1a70-4c07-8ccd-fe524703a84b?action=collection%2Ffork&source=rip_markdown&collection-url=entityId%3D58401244-916ddbda-1a70-4c07-8ccd-fe524703a84b%26entityType%3Dcollection%26workspaceId%3D8013472d-5897-47d1-bf5c-92683a027b35&utm_source=audiopod-docs-quickstart)

<Note>
The sandbox is rate-limited and capped at 200 characters per call — it's for a
first taste, not production. Ready for the full API? Continue below.
</Note>

## Choose Your Path

<Tabs>
  <Tab title="🖥️ cURL">
    Complete setup directly with cURL commands.
  </Tab>
  <Tab title="🌐 App">
    Login to AudioPod AI app to get your API key.
  </Tab>
</Tabs>

---

## Step 1: Create Your Account

<Tabs>
  <Tab title="CLI">
    ```bash
    curl -X POST "https://api.audiopod.ai/api/v1/auth/initiate-registration" \
      -H "Content-Type: application/json" \
      -d '{
        "email": "you@example.com",
        "password": "YourSecurePassword123",
        "full_name": "Your Name"
      }'
    ```
    
    ✅ Check your email for a 6-digit verification code.
  </Tab>
  <Tab title="Dashboard">
    <Card title="Sign Up" icon="user-plus" href="https://www.audiopod.ai/auth/signup">
      Create your free account
    </Card>
  </Tab>
</Tabs>

---

## Step 2: Verify Your Email

<Tabs>
  <Tab title="CLI">
    ```bash
    curl -X POST "https://api.audiopod.ai/api/v1/auth/verify-registration" \
      -H "Content-Type: application/json" \
      -d '{
        "email": "you@example.com",
        "verification_code": "123456"
      }'
    ```
    
    **Response includes your JWT token:**
    ```json
    {
      "access_token": "eyJhbGciOiJIUzI1NiIs...",
      "token_type": "bearer"
    }
    ```
    
    <Warning>Save the `access_token` — you need it for Step 3!</Warning>
  </Tab>
  <Tab title="Dashboard">
    Enter the 6-digit code from your email on the verification page.
  </Tab>
</Tabs>

---

## Step 3: Create Your API Key

<Tabs>
  <Tab title="CLI">
    ```bash
    # Use the access_token from Step 2
    curl -X POST "https://api.audiopod.ai/api/v1/auth/api-keys" \
      -H "Authorization: Bearer YOUR_ACCESS_TOKEN" \
      -H "Content-Type: application/json" \
      -d '{"name": "my-first-key", "scopes": ["*"]}'
    ```
    
    **Response:**
    ```json
    {
      "api_key": "ap_xxxxxxxxxxxxxxxxxxxxxxxx"
    }
    ```
    
    <Warning>**Save this key immediately!** It's only shown once.</Warning>
  </Tab>
  <Tab title="Dashboard">
    1. Go to [API Keys](https://www.audiopod.ai/dashboard/account/api-keys)
    2. Click **Create New API Key**
    3. Copy and save your key
  </Tab>
</Tabs>

---

## Step 4: Set Up Your Environment

```bash
# Add to your shell profile (~/.bashrc, ~/.zshrc)
export AUDIOPOD_API_KEY="ap_your_api_key_here"
```

Or create a `.env` file:

```bash
AUDIOPOD_API_KEY=ap_your_api_key_here
```

---

## Step 5: Add Funds to Your Wallet

<Tabs>
  <Tab title="CLI">
    ```bash
    # Get a Stripe payment link
    curl -X POST "https://api.audiopod.ai/api/v1/api-wallet/topup/checkout" \
      -H "X-API-Key: $AUDIOPOD_API_KEY" \
      -H "Content-Type: application/json" \
      -d '{"amount_cents": 500}'
    ```
    
    Open the returned `url` in your browser to complete payment.
  </Tab>
  <Tab title="Dashboard">
    1. Go to [API Keys](https://www.audiopod.ai/dashboard/account/api-keys)
    2. Click **Add Funds** in the Wallet Balance section
    3. Choose amount (min $1) and complete Stripe checkout
  </Tab>
</Tabs>

---

## Step 6: Make Your First API Call! 🎉

<Tabs>
  <Tab title="Python">
    ```bash
    pip install audiopod
    ```
    
    ```python
    from audiopod import AudioPod
    
    client = AudioPod()  # Uses AUDIOPOD_API_KEY env var
    
    # Separate a song into stems
    result = client.stems.separate(
        url="https://youtube.com/watch?v=dQw4w9WgXcQ",
        mode="six"
    )
    
    print("🎵 Stems ready!")
    for stem, url in result["download_urls"].items():
        print(f"  {stem}: {url[:50]}...")
    ```
  </Tab>
  <Tab title="Node.js">
    ```bash
    npm install audiopod
    ```
    
    ```typescript
    import AudioPod from 'audiopod';
    
    const client = new AudioPod();
    
    const result = await client.stems.separate({
      url: 'https://youtube.com/watch?v=dQw4w9WgXcQ',
      mode: 'six'
    });
    
    console.log('🎵 Stems ready!');
    Object.entries(result.download_urls).forEach(([stem, url]) => {
      console.log(`  ${stem}: ${url.substring(0, 50)}...`);
    });
    ```
  </Tab>
  <Tab title="cURL">
    ```bash
    # Separate audio into 6 stems
    curl -X POST "https://api.audiopod.ai/api/v1/stem-extraction/api/extract" \
      -H "X-API-Key: $AUDIOPOD_API_KEY" \
      -F "url=https://youtube.com/watch?v=dQw4w9WgXcQ" \
      -F "mode=six"
    
    # Check job status (replace JOB_ID)
    curl -X GET "https://api.audiopod.ai/api/v1/stem-extraction/status/JOB_ID" \
      -H "X-API-Key: $AUDIOPOD_API_KEY"
    ```
  </Tab>
</Tabs>

---

## Pricing

Pay only for what you use. No subscriptions required for API access.

One rate card: an API call costs exactly what the same job costs from the
dashboard, and the same number of credits comes off either way.

| Service | Rate | $10 Gets You |
|---------|------|--------------|
| Stem Separation | $0.088/min | 114 min |
| Transcription | $0.0293/min | 341 min |
| Voice Cloning/TTS | $66.67 per 1M characters | 150,000 characters |
| Voice Conversion | $0.132/min | 76 min |
| Music Generation | $0.132/min | 76 min |
| Speaker Separation | $0.044/min | 227 min |
| Audiobook Narration | $66.67 per 1M characters | 150,000 characters |
| Media Conversion | $0.0088/min | 1,136 min |

<Card title="Full Pricing" icon="calculator" href="/account/api-wallet">
  View complete pricing for all services
</Card>

---

## Try More Features

<Tabs>
  <Tab title="Transcription">
    ```python
    # Convert speech to text
    result = client.transcription.create(
        url="https://youtube.com/watch?v=VIDEO_ID"
    )
    print(result["transcript"])
    ```
  </Tab>
  <Tab title="Music Generation">
    ```python
    # Generate music from text
    song = client.music.generate(
        prompt="upbeat electronic dance music"
    )
    print(song["output_url"])
    ```
  </Tab>
  <Tab title="Voice Cloning">
    ```python
    # Clone a voice from audio
    voice = client.voice.clone(
        file="./sample.wav",
        name="My Voice"
    )
    
    # Generate speech with cloned voice
    audio = client.tts.generate(
        text="Hello world!",
        voice_id=voice["id"]
    )
    ```
  </Tab>
  <Tab title="Noise Reduction">
    ```python
    # Remove background noise
    clean = client.denoiser.denoise(
        file="./noisy_audio.wav"
    )
    print(clean["output_url"])
    ```
  </Tab>
</Tabs>

---

## Manage Your Account (via CLI)

Once you have an API key, manage everything via cURL:

```bash
# Check wallet balance
curl -s "https://api.audiopod.ai/api/v1/api-wallet/balance" \
  -H "X-API-Key: $AUDIOPOD_API_KEY" | jq .

# List your API keys
curl -s "https://api.audiopod.ai/api/v1/auth/api-keys" \
  -H "X-API-Key: $AUDIOPOD_API_KEY" | jq .

# Create another API key
curl -X POST "https://api.audiopod.ai/api/v1/auth/api-keys" \
  -H "X-API-Key: $AUDIOPOD_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"name": "production-key", "scopes": ["*"]}'

# View usage history
curl -s "https://api.audiopod.ai/api/v1/api-wallet/usage" \
  -H "X-API-Key: $AUDIOPOD_API_KEY" | jq .
```

---

## Common Issues

<AccordionGroup>
  <Accordion title="401 Unauthorized">
    Your API key is invalid or missing.
    
    **Fix:** Check the `X-API-Key` header is set correctly:
    ```bash
    echo $AUDIOPOD_API_KEY  # Should print your key
    ```
  </Accordion>
  <Accordion title="402 Insufficient Balance">
    Your wallet is empty.
    
    **Fix:** Add funds via CLI or dashboard:
    ```bash
    curl -X POST "https://api.audiopod.ai/api/v1/api-wallet/topup/checkout" \
      -H "X-API-Key: $AUDIOPOD_API_KEY" \
      -d '{"amount_cents": 500}'
    ```
  </Accordion>
  <Accordion title="Job Stuck in Processing">
    Audio processing jobs can take time depending on file length.
    
    **Fix:** Poll the status endpoint:
    ```bash
    curl "https://api.audiopod.ai/api/v1/stem-extraction/status/JOB_ID" \
      -H "X-API-Key: $AUDIOPOD_API_KEY"
    ```
  </Accordion>
</AccordionGroup>

---

## Building a startup?

**AudioPod for Startups** gives eligible early-stage teams free Pro access plus
API credits to build on. Apply at
[audiopod.ai/startups](https://www.audiopod.ai/startups).

## Next Steps

<Columns cols={3}>
  <Card title="SDK Docs" icon="code" href="/sdks/overview">
    Full SDK reference for Python & Node.js
  </Card>
  <Card title="API Reference" icon="book" href="/api-reference/stem-splitter">
    Detailed endpoint documentation
  </Card>
  <Card title="Pricing" icon="wallet" href="/account/api-wallet">
    Complete pricing & wallet management
  </Card>
  <Card title="Postman" icon="paper-plane" href="/sdks/postman">
    Fork the collections instead of copy-pasting
  </Card>
</Columns>
