...
## Step 3.5: Fund Your API Wallet

Before making API calls, add funds to your wallet:

1. Go to [API Keys Dashboard](https://www.audiopod.ai/dashboard/account/api-keys)
2. Click **Add Funds** in the Wallet Balance section
3. Choose an amount (minimum $1.00)
4. Complete payment via Stripe

<Info>
API calls are billed per minute of audio processed. Check [API Wallet pricing](/account/api-wallet) for rates.
</Info>
...
Separate music into individual instruments using simple mode names:
...
<Tabs>
  <Tab title="Python">
    ...
    # Submit 6-stem extraction job
    response = requests.post(
        f"{BASE_URL}/stem-extraction/api/extract",
        headers={"X-API-Key": API_KEY},
        data={
            "url": "https://youtube.com/watch?v=VIDEO_ID",
            "mode": "six"  # Options: single, two, four, six, producer, studio, mastering
        }
    )
    job = response.json()
    ...
    while True:
        status = requests.get(
            f"{BASE_URL}/stem-extraction/status/{job['id']}",
            headers={"X-API-Key": API_KEY}
        ).json()
        
        if status["status"] == "COMPLETED":
            print("Separated stems:")
            for stem, url in status["download_urls"].items():
                print(f"  {stem}: {url[:60]}...")
            break
  ...
  <Tab title="Node.js">
    ...
    const BASE_URL = 'https://api.audiopod.ai/api/v1';

    async function extractStems() {
      // Submit job
      const formData = new FormData();
      formData.append('url', 'https://youtube.com/watch?v=VIDEO_ID');
      formData.append('mode', 'six'); // Options: single, two, four, six, producer, studio, mastering

      const { data: job } = await axios.post(
        `${BASE_URL}/stem-extraction/api/extract`,
        formData,
        { headers: { 'X-API-Key': API_KEY } }
      );
      ...
      while (true) {
        const { data: status } = await axios.get(
          `${BASE_URL}/stem-extraction/status/${job.id}`,
          { headers: { 'X-API-Key': API_KEY } }
        );

        if (status.status === 'COMPLETED') {
          console.log('Separated stems:');
          Object.entries(status.download_urls).forEach(([stem, url]) => {
  ...
  <Tab title="cURL">
    ...
    # Available modes: single, two, four, six, producer, studio, mastering
    # For single mode, also specify: -F "stem=vocals"
...
**Available Modes:**
- `single` - Extract one stem (vocals, drums, bass, guitar, piano, or other)
- `two` - Vocals + Instrumental
- `four` - Vocals, Drums, Bass, Other
- `six` - Vocals, Drums, Bass, Guitar, Piano, Other
- `producer` - 8 stems with drum decomposition
