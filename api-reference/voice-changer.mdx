---
title: "Voice Changer"
description: "Transform voice characteristics in audio recordings using AudioPod AI's advanced voice-to-voice conversion technology."
---
...
def convert_podcast_guest_to_host_voice(guest_audio, host_voice_uuid, api_key):
    ...
    while True:
        status_response = requests.get(
            f"https://api.audiopod.ai/api/v1/voice/convert/{job_id}/status",
            headers={"X-API-Key": api_key}
        )
...
def localize_content_voice(source_audio, target_locale_voices, api_key):
    ...
    while len(completed_versions) < len(localized_versions):
        for locale, job_info in localized_versions.items():
            ...
            status_response = requests.get(
                f"https://api.audiopod.ai/api/v1/voice/convert/{job_info['job_id']}/status",
                headers={"X-API-Key": api_key}
            )
...
One rate card: **990 credits per minute** of source audio, whichever way you
authenticate. Funding from the API wallet costs the dollar value of those
credits at $1 = 7,500 credits.

| Path | Rate | Basis |
|------|------|-------|
| **Account credits** (subscription / JWT) | **990 credits per minute** | Duration of the source audio |
| **API wallet** (API key, USD) | **$0.132 per minute** | Duration of the source audio |
