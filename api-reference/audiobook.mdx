...
For a single manuscript parse, pass `auto_direction=true` or `auto_direction=false` explicitly to override the default without changing the saved preference. The parse endpoint accepts this in JSON, form fields, or the query string; an explicit body value takes precedence over the query value.
