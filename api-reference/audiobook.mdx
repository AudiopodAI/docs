...
The API-key workflow supports asynchronous production: submit a job, get an ID back immediately, and receive a signed webhook when it finishes — so your control panel never blocks on a long render. The guided narrator-sample endpoint described below is Studio-only.
...
## Account audiobook settings

`GET /api/v1/audiobook/settings` reads the authenticated account's preference for automatic reading direction in Audiobook Studio. `PATCH /api/v1/audiobook/settings` saves it and returns the same response shape.

```bash
# Read the account preference using session authentication
curl "https://api.audiopod.ai/api/v1/audiobook/settings" \
  -H "Authorization: Bearer $ACCESS_TOKEN"

# Turn automatic direction off for future Studio parses
curl -X PATCH "https://api.audiopod.ai/api/v1/audiobook/settings" \
  -H "Authorization: Bearer $ACCESS_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"auto_direction_default": false}'
```

The PATCH body requires `auto_direction_default`, a boolean. Both endpoints return:

```json
{
  "auto_direction_default": false,
  "source": "account",
  "active": true,
  "applies_to_api_key_parses": false
}
```

| Field | Meaning |
|-------|---------|
| `auto_direction_default` | The author's saved preference. It is `true` if the account has never chosen. |
| `source` | `account` after an explicit choice; `default` if the account has never chosen. |
| `active` | Whether AudioPod currently applies account preferences to Studio parses. This is separate from the author's choice. |
| `applies_to_api_key_parses` | Always `false`. API-key parses remain opt-in, regardless of this preference. |

When `active` is `false`, PATCH still saves the preference and returns the saved value. Studio parses with no explicit `auto_direction` then default to `false`; the saved choice takes effect when `active` becomes `true`. Changing this preference does not direct or re-render existing books.

For a single manuscript import, pass `auto_direction=true` or `auto_direction=false` explicitly to override the default without changing the saved preference. This applies to uploaded-file parsing, pasted text, and generated manuscripts. Omit the field or pass JSON `null` to use the default.

The uploaded-file parse endpoint accepts this in JSON, form fields, or the query string; an explicit body value takes precedence over the query value. The paste and generate endpoints accept it in their JSON bodies.

### Guided narrator samples in Studio

The Studio-only `POST /api/v1/audiobook/projects/{project_id}/production/quote` request also accepts an optional `auto_direction` boolean. Omitted or `null` follows the author's account preference; an account with no saved preference keeps the guided workflow's default of `true`. An explicit `true` or `false` overrides that choice for this production.

This guided sample default applies independently of the `active` field for automatic direction during manuscript intake. A saved `false` is respected in both workflows. API-key requests to the guided quote endpoint return `400`; use the API narration workflow documented below.

Preparing a guided sample takes over any outstanding automatic direction started by manuscript intake, so the sample does not wait for a whole-book direction pass. A direction run started explicitly is preserved: finish or stop it before preparing the sample, otherwise the quote request returns `409`.
...
|-----------|--------|----------|
| Account audiobook settings | GET·PATCH | `/audiobook/settings` |
...
<Note>
**API-key parses default to `auto_direction=false`.** Set `auto_direction=true` explicitly to add reading direction after parsing. The account preference does not change this API-key default.

In Audiobook Studio, an omitted `auto_direction` follows the author's [account preference](#account-audiobook-settings) when that setting is active. An explicit `true` or `false` overrides the default for that parse, for either authentication method. Explicit `true` also works when the account preference is not active.
...
<ParamField body="auto_direction" type="boolean | null">Add automatic reading direction after splitting the text into chapters. Omitted or `null` follows the [account preference](#account-audiobook-settings) for Studio requests and defaults to `false` for API keys. An explicit `true` or `false` overrides that default for this import.</ParamField>
...
<ParamField body="auto_direction" type="boolean | null">Add automatic reading direction after generating and splitting the manuscript. Omitted or `null` follows the [account preference](#account-audiobook-settings) for Studio requests and defaults to `false` for API keys. An explicit `true` or `false` overrides that default for this import.</ParamField>
...
When automatic direction is requested, it continues asynchronously after the manuscript is saved. Read `GET /audiobook/projects/{project_id}/direction/auto/latest` to find its job and follow [direction progress](#waiting-for-a-direction-run). Direction does not start audio narration; submit a narration request when you are ready.
