...
Cloning is **instant** — a custom voice is ready to use in **seconds** from a
single short reference clip. Provide **5–30 seconds** of clean speech and the
returned voice can be used immediately for text-to-speech and voice conversion.
...
<Tabs>
  <Tab title="POST">
    ...
    name: My Instant Clone
    file: (5-30s reference audio)
    ```
  </Tab>
  <Tab title="Python">
    ```python
    ...
    with open("reference_5s.wav", "rb") as audio_file:
        response = requests.post(
            "https://api.audiopod.ai/api/v1/voice/voice-clone",
            headers={"X-API-Key": api_key},
            data={"name": "My Instant Clone"},
            files={"file": audio_file},
        )

    voice = response.json()
    print(f"Voice ready: {voice['id']} — status {voice['status']}")
    ```
  </Tab>
  <Tab title="cURL">
    ```bash
    curl -X POST "https://api.audiopod.ai/api/v1/voice/voice-clone" \
      -H "X-API-Key: $AUDIOPOD_API_KEY" \
      -F "name=My Instant Clone" \
      -F "file=@reference_5s.wav"
    ```
  </Tab>
</Tabs>
...
[Text to Speech](/api-reference/text-to-speech) or
[Voice Changer](/api-reference/voice-changer).
</Note>

## Design a Voice

Don't have a reference recording? **Design a voice from a text description.**
Describe the voice you want and get back audition-ready candidate previews, then
publish the one you like into a durable custom voice.

<Note>
Voice Design is a **paid-tier feature** (Creator and above). Each generated
...
<Tabs>
  <Tab title="POST">
    ...
    {
      "design_prompt": "A warm, middle-aged British narrator with a calm, unhurried delivery",
      "preview_text": "This is a preview of the voice you just designed.",
      "num_samples": 3
    }
    ```
  </Tab>
  <Tab title="Python">
    ...
    response = requests.post(
        "https://api.audiopod.ai/api/v1/voice/design",
    ...
    data = response.json()
    print(f"Charged {data['credits_charged']} credits")
    for preview in data["previews"]:
        print(f"{preview['preview_id']}: {preview['audio_url']}")
    ```
  </Tab>
</Tabs>
...
```json
{
  "previews": [
    {
      "preview_id": "b1f2c3d4-....",
      "audio_url": "https://presigned-url-to-preview.mp3",
...
Turn a chosen preview into a permanent custom voice with
`POST /api/v1/voice/design/publish`. Publishing is free and the resulting voice
works everywhere a cloned voice does.

<Tabs>
  <Tab title="POST">
    ```http
    POST /api/v1/voice/design/publish
    X-API-Key: {api_key}
    Content-Type: application/json

    {
      "preview_id": "b1f2c3d4-....",
      "name": "Calm British Narrator",
      "description": "Designed voice for long-form narration"
    }
    ```
  </Tab>
  <Tab title="Python">
    ```python
    response = requests.post(
        "https://api.audiopod.ai/api/v1/voice/design/publish",
        headers={"X-API-Key": api_key, "Content-Type": "application/json"},
        json={
            "preview_id": "b1f2c3d4-....",
            "name": "Calm British Narrator",
    ...
    voice = response.json()
    print(f"Published voice: {voice['id']} ({voice['name']})")
    ```
  </Tab>
</Tabs>
...
<AccordionGroup>
  ...
  <Accordion title="402 CUSTOM_VOICE_LIMIT_REACHED - Voice Slot Limit">
    ...
    {
      "detail": {
        "code": "CUSTOM_VOICE_LIMIT_REACHED",
        "message": "You've used 3 of 3 custom voices on the Basic plan. Delete a voice or upgrade for more.",
        "feature": "custom_voices",
        "current_plan": "basic",
        "limit": 3,
        "used": 3
      }
    ...
    **Solutions:** - Delete an unused custom voice to free a slot - Upgrade to a paid plan for unlimited custom voices

    Check your current usage with `GET /api/v1/voice/voice-profiles/limits`:

    ```json
    { "used": 3, "limit": 3, "unlimited": false, "plan": "basic" }
    ```
