...
| Mode | Output | Plan | Use Case |
|------|--------|------|----------|
| `single` | One named stem + everything-else | Basic | Vocal isolation, drum extraction |
| `two` | vocals + instrumental | Basic | Karaoke tracks |
| `four` | vocals, drums, bass, other | Basic | Standard remixing |
| `six` | + guitar, piano | Basic | Full instrument separation |
| `producer` | Drums split into kick, snare, hi-hat — up to 8 | Creator | Beat production |
| `studio` | Full drum kit plus a bass frequency split — up to 12 | Pro | Professional mixing |
| `mastering` | Full kit, bass split, and vocal stereo separation — up to 16 | Pro | Maximum detail |

<Info>
**Counts are a ceiling, not a promise.** `producer`, `studio`, and `mastering`
decompose whatever is actually in the recording, so the stem count depends on
the track — a song with no cymbals yields no cymbal stem. Plan for "up to N".
</Info>

<Warning>
**Plan gating.** `producer` requires **Creator** or above; `studio` and
`mastering` require **Pro** or above. A request from a lower plan is rejected —
call `GET /api/v1/stem-extraction/packages/available` to see what the
authenticated key can use before you dispatch a job.
</Warning>

For `mode="single"` this endpoint accepts the six core stems — `vocals`,
...
Beyond the fixed modes, you can name **one** instrument out of a 45-entry
catalog and get that instrument back plus an everything-else track. The 45 is
the size of the menu you can choose from — not the number of files a job
returns.

Use `POST /api/v1/stem-extraction/extract/advanced` with `single_stem` set to a
...
<Tabs>
  <Tab title="cURL">
    ```bash
    # Isolate the saxophone from a track
    curl -X POST https://api.audiopod.ai/api/v1/stem-extraction/extract/advanced \
      -H "X-API-Key: $AUDIOPOD_API_KEY" \
      -F "url=https://youtube.com/watch?v=VIDEO_ID" \
      -F "single_stem=saxophone"
    ```
  </Tab>
  <Tab title="Exclude one instrument">
    ```bash
    # Everything EXCEPT the saxophone (JSON array, one id)
    curl -X POST https://api.audiopod.ai/api/v1/stem-extraction/extract/advanced \
      -H "X-API-Key: $AUDIOPOD_API_KEY" \
      -F "url=https://youtube.com/watch?v=VIDEO_ID" \
      -F 'exclude_stems=["saxophone"]'
    ```
  </Tab>
</Tabs>
...
Don't hard-code the list — plan gating and catalog contents change. `GET
/api/v1/stem-extraction/catalog` returns every stem with the plan it needs and
whether the authenticated key can use it right now:

```bash
curl -s https://api.audiopod.ai/api/v1/stem-extraction/catalog \
  -H "X-API-Key: $AUDIOPOD_API_KEY"
```

Each entry carries an `id` (what you send as `single_stem`), a `label` and
...
**Core — available on every plan including Basic (7)**

`vocals` · `backing_track` · `drums` · `bass` · `guitar` · `piano` · `other`
...
<Warning>
Requesting a Creator-tier stem with a Basic key is rejected. Check `allowed` on
the catalog response first, or handle the error and prompt for an upgrade.
</Warning>
...
[API Wallet & Pricing](/account/api-wallet).
</Info>
...
**Base rate:** 660 credits ($0.088) per minute of audio processed for every
standard mode — `single`, `two`, `four`, `six`, `producer`, `studio`,
`mastering`. The same job costs the same from the dashboard or from an API key.

| Duration | Credits | Cost (standard modes) |
|----------|---------|-----------------------|
| 3 minutes | 1,980 | $0.27 |
| 5 minutes | 3,300 | $0.44 |
| 10 minutes | 6,600 | $0.88 |
| 1 hour | 39,600 | $5.28 |

### Premium catalog selections

Some catalog selections run on a higher-fidelity engine and bill at a multiplier
of the base rate:

| Selection | Multiplier | Effective rate |
|-----------|------------|----------------|
| Lead / backing vocal split (`lead_vocal`, `backing_vocal`) | **1.5×** | $0.132/min |
| Vocal gender split (`male_vocal`, `female_vocal`) | **1.5×** | $0.132/min |
| Single-instrument isolation from the 45-instrument catalog | **2.0×** | $0.176/min |
