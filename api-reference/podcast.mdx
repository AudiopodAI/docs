---
title: "Podcast"
description: "Generate multi-speaker podcast episodes from your source material — outline, transcript, and narrated audio."
---
...
transcript, then render the narrated audio. The RSS feed is limited while it
is rebuilt; see [RSS feed](#rss-feed-limited) below.
...
| 7. RSS feed (limited) | `POST /api/v1/podcast/feed` |
...
<Warning>
The RSS feed is being rebuilt and is limited until then. Today it lists every
completed episode on your account, and anyone with the feed URL can read it.
Listing is moving to per-episode opt-in. The feed has no show artwork yet, so
podcast directories such as Apple Podcasts and Spotify will not accept it. To
publish an episode today, download its audio and upload it to your podcast
host.
</Warning>

The feed URL carries a signed, feed-scoped token.
