...
AudioPod AI's Text to Speech service provides unified text-to-speech capabilities for both **standard pre-built voices** and **custom voice clones**. Generate speech with any of **500+ voices** across **100+ languages** using our advanced audio AI stack.
...
- **500+ Voices**: Pre-built voices with unique characteristics, plus your own custom clones
- **Inline Directing (TTS-2)**: Steer emotion, delivery, non-verbal sounds, pauses, and pronunciation right inside the text — on every plan
...
|-----------|--------|----------|
| List voices | GET | `/api/v1/voice/voice-profiles` |
| Generate TTS | POST | `/api/v1/voice/voices/{voice_id}/generate` |
| Get job status | GET | `/api/v1/voice/tts-jobs/{job_id}/status` |

## Quick Start (cURL)

```bash
# 1. List available voices to get a voice UUID
curl -X GET "https://api.audiopod.ai/api/v1/voice/voice-profiles?include_public=true&limit=10" \
  -H "X-API-Key: $AUDIOPOD_API_KEY"

# 2. Generate speech (replace VOICE_UUID with actual UUID from step 1)
curl -X POST "https://api.audiopod.ai/api/v1/voice/voices/VOICE_UUID/generate" \
  -H "X-API-Key: $AUDIOPOD_API_KEY" \
  -H "Content-Type: application/x-www-form-urlencoded" \
  -d "input_text=Hello, this is a test." \
  -d "audio_format=mp3" \
  -d "speed=1.0"
# Returns: {"job_id": 1234, "status": "pending", ...}

# 3. Poll job status until completed
curl -X GET "https://api.audiopod.ai/api/v1/voice/tts-jobs/1234/status" \
  -H "X-API-Key: $AUDIOPOD_API_KEY"
# Returns: {"status": "completed", "output_url": "https://...", ...}

# 4. Download the audio from output_url
curl -o output.mp3 "OUTPUT_URL_FROM_STEP_3"
```
...
**Parameters** (submit as `application/x-www-form-urlencoded` form fields):
- `voice_id` (required): Voice UUID, ID, or name from your voice collection (path parameter)
- `input_text` (required): Text to convert to speech (max **20,000 characters** per request; may include inline directing — see below)
- `audio_format` (optional): Output format - mp3, wav, ogg (default: mp3)
- `speed` (optional): Speech speed 0.25-4.0 (default: 1.0)
- `language` (optional): Language code - auto-detected if not provided
- `generation_params` (optional): JSON string of TTS-2 controls — `delivery_mode`, `timestamp_type`, `text_normalization` (see [Delivery & timestamp controls](#delivery-timestamp-controls))
...
## Inline Directing (TTS-2)

Steer the performance directly inside `input_text` — no separate parameters
needed. Inline directing is available on **every plan**, including the free tier.

### Emotion & delivery cues

Lead a segment with a bracketed direction of one or two words. It sets the mood
for the text that follows, until the next cue.

```text
[whispering, tense] Did you hear that? [excited] It's finally here!
```

### Non-verbal sounds

Drop a bracketed tag where you want a natural, non-verbal sound:

```text
That's hilarious [laugh]. Okay [sigh], let's get back to it.
```

Supported tags: `[laugh]` `[sigh]` `[clear throat]` `[breathe]` `[cough]`
`[yawn]` `[chuckle]` `[gasp]` `[groan]`.

### Pauses

Insert a timed silence with a self-closing break tag. Each pause is capped at
**10 seconds**, and you can use up to **20 breaks per request**.

```text
Let me think about that <break time="500ms"/> yes, that works.
```

### Pronunciation (inline IPA)

Wrap a word's [IPA](https://en.wikipedia.org/wiki/International_Phonetic_Alphabet)
pronunciation in forward slashes to override how it's spoken:

```text
Welcome to /ˈwʊstər/ — you'd never guess it from the spelling.
```
...
<Tabs>
  <Tab title="cURL">
    ...
    curl -X POST "https://api.audiopod.ai/api/v1/voice/voices/VOICE_UUID/generate" \
      -H "X-API-Key: $AUDIOPOD_API_KEY" \
      -H "Content-Type: application/x-www-form-urlencoded" \
      --data-urlencode 'input_text=[warm] Hello there [laugh]. Welcome to /ˈwʊstər/ <break time="600ms"/> let'\''s begin.' \
      -d 'audio_format=mp3'
    ```
  </Tab>
  <Tab title="Python">
    ...
    directed_text = (
        "[warm] Hello there [laugh]. "
        'Welcome to /ˈwʊstər/ <break time="600ms"/> '
        "let's begin."
    )
    ...
    response = requests.post(
        ...
        headers={"X-API-Key": api_key},
        data={"input_text": directed_text, "audio_format": "mp3"},
    )
...
Pass a JSON string in the `generation_params` form field to tune delivery and
request a timestamp sidecar.

| Key | Values | Description |
|-----|--------|-------------|
| `delivery_mode` | `stable`, `balanced`, `creative` | How expressive vs. consistent the read is. `stable` hugs the text closely; `creative` gives the voice more range. |
| `timestamp_type` | `word`, `character` | Emit follow-along timestamps at word or character granularity. |
| `text_normalization` | `true`, `false` | Expand numbers, dates, and abbreviations into spoken form. |

```bash
curl -X POST "https://api.audiopod.ai/api/v1/voice/voices/VOICE_UUID/generate" \
  -H "X-API-Key: $AUDIOPOD_API_KEY" \
  -H "Content-Type: application/x-www-form-urlencoded" \
  --data-urlencode 'input_text=The meeting is at 3:30 on 04/09.' \
  --data-urlencode 'generation_params={"delivery_mode":"balanced","timestamp_type":"word","text_normalization":true}'
```

### Timestamps

When you request `timestamp_type`, the completed job response includes a
...
```json
{
  ...
  "timestamps_url": "https://presigned-url-to-timestamps-sidecar.json",
  "total_duration": 4.2
}
```

Each entry pairs a token with its `start`/`end` time in seconds:
...
```json
{
  ...
  "tokens": [
    ...
    { "text": "meeting", "start": 0.18, "end": 0.61 }
  ]
}
```
...
AudioPod AI supports 100+ languages with automatic detection and consistent quality across all supported languages:
...
```python
intro_script = (
    "[speak with warm upbeat energy] Welcome back to the Tech Talk Podcast! "
    '<break time="1s"/> '
    "Today we're diving deep into artificial intelligence and its impact on "
    'content creation. <break time="0.5s"/> Let\'s get started!'
)

response = requests.post(
    "https://api.audiopod.ai/api/v1/voice/voices/ava/generate",
    headers={"Authorization": f"Bearer {API_KEY}"},
    data={
        "input_text": intro_script,
        "audio_format": "mp3",
        # Expressive read; see "Delivery & timestamp controls" above.
        "generation_params": '{"delivery_mode":"creative"}',
    },
)
job_id = response.json()["job_id"]  # poll to COMPLETED, then download audio_url
```
...
def create_lesson_audio(lesson_content):
    # Direct the pacing with a leading note + timed pauses (not SSML).
    directed = (
        "[speak clearly and calmly at a measured pace] Lesson begins now. "
        '<break time="1s"/> '
        f"{lesson_content} "
        '<break time="2s"/> '
        "That concludes this lesson. Take a moment to review what you've learned."
    )
    return requests.post(
        "https://api.audiopod.ai/api/v1/voice/voices/sage/generate",
        headers={"Authorization": f"Bearer {API_KEY}"},
        data={
            "input_text": directed,
            "audio_format": "mp3",
            "generation_params": '{"delivery_mode":"stable"}',
        },
    )
```
...
for prompt_name, text in ivr_prompts.items():
    response = requests.post(
        "https://api.audiopod.ai/api/v1/voice/voices/ava/generate",
        headers={"Authorization": f"Bearer {API_KEY}"},
        data={"input_text": text, "audio_format": "wav"},
    )
    # Poll response.json()["job_id"] to COMPLETED, then download its audio_url.
```
...
<AccordionGroup>
  <Accordion title="400 Bad Request - Invalid Text">
    **Causes:** - Text too long (>20,000 characters) - Invalid characters or
    encoding - Empty text field - Malformed `generation_params` JSON **Solutions:** - Split long text into chunks -
    Check text encoding (UTF-8) - Validate text is not empty - Verify `generation_params` is a valid JSON object
  </Accordion>
...
| Path | Rate | Basis |
|------|------|-------|
| **Account credits** (subscription / JWT) | **0.5 credits per character** | Input character count, charged before synthesis |
| **API wallet** (API key, USD) | **$0.04 per minute** | Duration of the generated audio |
...
| ----------- | ------------ |
| 100 characters | 50 credits |
| 500 characters | 250 credits |
| 1,000 characters | 500 credits |
| 5,000 characters | 2,500 credits |

### Cost Examples — API wallet (per minute of output)

| Output Duration | USD Cost |
| --------------- | -------- |
| 30 seconds | $0.02 |
| 1 minute | $0.04 |
| 5 minutes | $0.20 |
