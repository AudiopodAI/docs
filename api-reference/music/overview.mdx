...
AudioPod AI's **AudioMusic V2** is a state-of-the-art music generation engine that transforms text descriptions into original musical compositions. Powered by a Diffusion Transformer (DiT) and a 4B parameter Language Model with Chain-of-Thought reasoning, AudioMusic V2 delivers studio-quality results across 50+ languages.

### Key Capabilities

- **Text-to-Music**: Generate complete songs with vocals from prompts and lyrics
- **Simple Mode**: Describe what you want in natural language — the 4B LM handles everything
...
<Tabs>
  <Tab title="Python">
    ...
    # Simple mode — the 4B LM auto-generates everything
...
### Language Model (4B LM) Parameters

AudioMusic V2 includes a 4B parameter language model that can auto-generate and refine metadata, captions, lyrics, and more.
...
These are auto-detected by the 4B LM when `thinking: true`, but can be specified manually.
...
1. **Use Simple Mode for quick results** — describe what you want in natural language and let the 4B LM handle the details
...
<Columns cols={2}>
  ...
  <Card title="Social & Sharing" icon="share-nodes" href="/api-reference/music/social">
    Like, share, comments, featured tracks, Music Radio
  </Card>
</Columns>
